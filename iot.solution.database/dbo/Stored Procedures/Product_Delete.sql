...
001	SVM-4 15-05-2020 [Nishit Khakhi]	Added Initial Version to Delete Product
*******************************************************************/
...
BEGIN
	...
	BEGIN
        ...
        SELECT @Param = 
        (
            SELECT 'Product_Delete' AS '@procName'             
    ...
    BEGIN TRY            
		...
		IF NOT EXISTS (SELECT TOP 1 1 FROM [dbo].[Product] (NOLOCK) WHERE [companyGuid] = @companyguid AND [guid] = @guid AND [isdeleted] = 0)
		BEGIN
			SET @output = -3
			SET @fieldname = 'ProductNotFound'
			RETURN;
		END
