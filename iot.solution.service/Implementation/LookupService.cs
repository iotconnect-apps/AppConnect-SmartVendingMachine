...
namespace iot.solution.service.Data
{
    public class LookupService : ILookupService
    {
        ...
        private readonly IDeviceItemRepository _deviceItemRepository;
        ...
        public LookupService(LogHandler.Logger logManager, IDeviceRepository deviceRepository,IkitTypeCommandRepository kitTypeCommandRepository
           ...
           , IKitTypeRepository kitTypeRepository, IProductTypeRepository productTypeRepository, 
            IProductRepository productRepository, IKitTypeAttributeRepository kitTypeAttributeRepository,IDeviceItemRepository deviceItemRepository, IInventoryRepository inventoryRepository)
        {
            _logger = logManager;
        ...
        public List<Entity.LookupItem> Get(string type, string param)
        {
            ...
            switch (type.ToLower())
            {
                ...
                case "producttype":
                    result = new List<Entity.LookupItem>();
                    result = (from producttype in _productTypeRepository.GetAllProductTypes().Where(r => r.IsActive.HasValue && r.IsActive.Value) 
                              select new Entity.LookupItem()
                              {
                                  Text =  producttype.TypeName,
                                  Value = producttype.Guid.ToString().ToUpper()
                              }).ToList();
                    break;
                //case "location":
                //    if (string.IsNullOrWhiteSpace(param)) { throw new System.Exception("Companyid is missing in request"); }
        ...
        public List<Entity.LookupItem> ProductLookup(Guid productTypeId)
        {
            List<Entity.LookupItem> result = new List<Entity.LookupItem>();
            var template = _companyRepository.FindBy(t => t.Guid == SolutionConfiguration.CompanyId).FirstOrDefault();
            if (template != null)
            {
                result = (from t in _productRepository.GetAll() 
                          where t.CompanyGuid.Equals(SolutionConfiguration.CompanyId) && t.ProductTypeGuid.Equals(productTypeId) && !t.IsDeleted
                          select new Entity.LookupItem()
                          {
                              Text = t.Name,
                              Value = t.Guid.ToString()
        ...
        public List<Entity.LookupItem> ProductTypeLookup(Guid deviceId)
        {
            List<Entity.LookupItem> result = new List<Entity.LookupItem>();
            var template = _companyRepository.FindBy(t => t.Guid == SolutionConfiguration.CompanyId).FirstOrDefault();
            if (template != null)
            {
                var ids= _deviceItemRepository.FindBy(x => x.DeviceGuid == deviceId && !x.IsDeleted).Select(p => p.ProductTypeGuid).Distinct().ToList();
                result = (from t in _productTypeRepository.GetAll() where ids.Contains(t.Guid)
                          select new Entity.LookupItem()
                          {
                              Text = t.TypeName,
                              Value = t.Guid.ToString()
                          }).ToList();
                //result = (from t in _deviceItemRepository.GetAll()
                //          join type in _productTypeRepository.GetAll() on t.ProductTypeGuid equals type.Guid
                //          where t.DeviceGuid.Equals(deviceId) && !t.IsDeleted
                //          select new Entity.LookupItem()
                //          {
                //              Text = type.TypeName,
                //              Value = type.Guid.ToString()
        ...
        public List<Entity.LookupItem> DeviceProductLookup(Guid deviceId)
        {
            List<Entity.LookupItem> result = new List<Entity.LookupItem>();
            var template = _companyRepository.FindBy(t => t.Guid == SolutionConfiguration.CompanyId).FirstOrDefault();
            if (template != null)
            {                
                result = (from t in _inventoryRepository.GetAll().Where(t=>!t.IsDeleted)
                          join product in _productRepository.GetAll().Where(t => !t.IsDeleted) on t.ProductGuid equals product.Guid 
                          where t.DeviceGuid.Equals(deviceId)
                          select new Entity.LookupItem()
                          {
                              Text = product.Name,
                              Value = product.Guid.ToString()
                          }).ToList();
        ...
        public long GetShelfCapacity(Guid shelfId)
        {
            long result = 0;
            var template = _companyRepository.FindBy(t => t.Guid == SolutionConfiguration.CompanyId).FirstOrDefault();
            if (template != null)
            {
                var capacity = _deviceItemRepository.FindBy(x => x.Guid.Equals(shelfId)).Select(x => x.Capacity).FirstOrDefault();
                if(capacity != null)
                {
                    result = Convert.ToInt64(capacity);
                }
            }
            return result;
        }
        public List<Entity.LookupItem> ShelfLookup(Guid deviceId,Guid productTypeId)
        {
            List<Entity.LookupItem> result = new List<Entity.LookupItem>();
            var template = _companyRepository.FindBy(t => t.Guid == SolutionConfiguration.CompanyId).FirstOrDefault();
            if (template != null)
            {
               result = (from t in _deviceItemRepository.GetAll()
                          where t.DeviceGuid.Equals(deviceId) && t.ProductTypeGuid.Equals(productTypeId) && !t.IsDeleted
                          select new Entity.LookupItem()
                          {
                              Text = t.UniqueId,
                              Value = t.Guid.ToString()
