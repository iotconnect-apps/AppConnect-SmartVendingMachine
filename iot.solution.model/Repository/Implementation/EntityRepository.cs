...
namespace iot.solution.model.Repository.Implementation
{
    public class EntityRepository : GenericRepository<Model.Entity>, IEntityRepository
    {
        ...
        public Entity.SearchResult<List<Entity.EntityDetail>> List(Entity.SearchRequest request)
        {
            ...
            List<Entity.EntityDetail> lst = new List<EntityDetail>();
            try
            {
                ...
                using (var sqlDataAccess = new SqlDataAccess(ConnectionString))
                {
                    ...
                    lst = DataUtils.DataReaderToList<Entity.EntityDetail>(dbDataReader, null);
                    foreach (var item in lst)
                    {
                        if(item.DeviceXML!=null)
                        {
                            //List<Entity.device> obj = new List<device>();
                            //XmlSerializer serializer = new XmlSerializer(typeof(List<Entity.device>));
                            //StringReader stringReader = new StringReader(item.DeviceXML.ToString());
                            //obj = (List<Entity.device>)serializer.Deserialize(stringReader);
                            //item.EntityDevices = obj;

                            XmlSerializer serializer = new XmlSerializer(typeof(Entity.devices));
                            StringReader stringReader = new StringReader(item.DeviceXML.ToString());
                            Entity.devices userRight = (Entity.devices)serializer.Deserialize(stringReader);
                            item.EntityDevices = userRight.devicelist;
                        }
                    }
                    result.Items = lst;
